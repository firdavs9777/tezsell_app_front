...
class _WriteReviewScreenState extends ConsumerState<WriteReviewScreen> {
  ...
  String? _resolvedCounterpartyAvatar;
  ...
  bool get _isBuyerReview => widget.isBuyerReview ?? _resolvedIsBuyerReview ?? true;
  ...
  String? get _counterpartyAvatar => widget.counterpartyAvatar ?? _resolvedCounterpartyAvatar;
  ...
  Future<void> _resolveTransactionDetails() async {
    ...
    if (tx != null) {
      ...
      final isBuyer = currentUserId != null && currentUserId == tx.buyer.id;
      final counterparty = isBuyer ? tx.seller : tx.buyer;
      setState(() {
        _resolvedIsBuyerReview = isBuyer;
        _resolvedCounterpartyName =
            counterparty.username.isNotEmpty ? counterparty.username : null;
        _resolvedCounterpartyAvatar = counterparty.avatar;
  ...
  Future<void> _submit() async {
    ...
    final error = ref.read(reviewsProvider).error;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(error ?? l?.reviewWriteError ?? 'Failed to submit review. Please try again.'),
        behavior: SnackBarBehavior.floating,
  ...
  Widget build(BuildContext context) {
    ...
    final canSubmit = _rating >= 1 && !_isSubmitting;
